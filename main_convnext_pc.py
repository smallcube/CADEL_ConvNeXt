import argparse
from run_networks_convnext import model
import yaml
import os
import pprint
...
def get_args_parser():
    ...
    parser.add_argument('--output_dir', default="", help='path where to save, empty for not saving')
...
if __name__=='__main__':
    ...
    # LOAD CONFIGURATIONS
    config_path = './config/ImageNet_LT/convnext_stage2.yaml'
    ...
    training_model = model(config, args)

    best_tao = 1
    best_acc = 0
    taos = [i for i in range(0.9, 1.5, 0.1)]
    for tao in taos:
        print("tao=", tao)
        rsl = training_model.eval(phase='val', tao=tao, post_hoc=True)
        if rsl['val-all']>best_acc:
            best_acc = rsl['val_all']
            best_tao =tao

    acc = training_model.eval(phase='test', tao=best_tao, post_hoc=True)
